...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack=[]#to push just append. to pop just use 
    ...
    def parse_step(self, transition):
        ...
        if transition=="S":
            ...
            f_w=self.stack[-1]
            s_w=self.stack.pop(-2)
            self.dependencies.append((f_w,s_w))
        elif transition=='RA':
            s_w=self.stack[-2]
            f_w=self.stack.pop()
            self.dependencies.append((s_w,f_w))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(len(unfinished_parses)!=0):
        ...
        for parser,action in zip(unfinished_parses[:batch_size],transitions):
