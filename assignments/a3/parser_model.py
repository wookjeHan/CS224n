...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        embed_to_hidden_weight=torch.zeros(self.n_features*self.embed_size,self.hidden_size)
        nn.init.xavier_uniform_(embed_to_hidden_weight)
        self.embed_to_hidden_weight=nn.Parameter(embed_to_hidden_weight)

        embed_to_hidden_bias=torch.zeros(self.hidden_size)
        nn.init.uniform_(embed_to_hidden_bias)
        self.embed_to_hidden_bias=nn.Parameter(embed_to_hidden_bias)
        
        self.dropout=nn.Dropout(p=dropout_prob)

        hidden_to_logits_weight=torch.empty(self.hidden_size,self.n_classes)
        nn.init.xavier_uniform_(hidden_to_logits_weight)
        self.hidden_to_logits_weight=nn.Parameter(hidden_to_logits_weight)
        
        hidden_to_logits_bias=torch.empty(self.n_classes)
        nn.init.uniform_(hidden_to_logits_bias)
        self.hidden_to_logits_bias=nn.Parameter(hidden_to_logits_bias)
    ...
    def embedding_lookup(self, w):
        ...
        x=torch.empty(batch_size,n_features,self.embed_size)
        x=self.embeddings[w]
        x=x.view(batch_size,n_features*self.embed_size)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        iptW=self.embedding_lookup(w)
        FC1=torch.matmul(iptW,self.embed_to_hidden_weight)+self.embed_to_hidden_bias
        nn.functional.relu_(FC1)
        D1=self.dropout(FC1)
        logits=torch.matmul(D1,self.hidden_to_logits_weight)+self.hidden_to_logits_bias
