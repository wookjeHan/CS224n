...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sentence in sents:
            ...
            for word in sentence:
                chars=[]
                chars.append(self.start_of_word)
                for i in word:
                    chars.append(self.char2id[i])
                chars.append(self.end_of_word)
                charIndicies.append(chars)
            sentenceIndicies.append(charIndicies)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        befFixed=self.words2charindices(sents)
        afFixed=pad_sents_char(befFixed,0)
        wordEmbed=torch.tensor(afFixed,device=device)
        wordEmbed=wordEmbed.transpose(0,1)
        return wordEmbed
