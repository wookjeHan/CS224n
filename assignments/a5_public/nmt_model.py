...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ipt=nn.utils.rnn.pack_padded_sequence(X,source_lengths)
        hiddens,(h_n,c_n)=self.encoder(ipt)
        enc_hiddens,_=nn.utils.rnn.pad_packed_sequence(hiddens,batch_first=True)
        init_hidden=torch.cat((h_n[0],h_n[1]),dim=1)
        init_decoder_hidden=self.h_projection(init_hidden)
        init_cell=torch.cat((c_n[0],c_n[1]),dim=1)
        init_cell_hidden=self.c_projection(init_cell)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        unSqueezedHidden=torch.unsqueeze(dec_hidden,2)
        e_t=torch.bmm(enc_hiddens_proj,unSqueezedHidden)
        e_t=torch.squeeze(e_t,2)#shape (b,src_len)
        ...
        unSqueezedAlpha=torch.unsqueeze(alpha_t,1)
        a_t=torch.bmm(unSqueezedAlpha,enc_hiddens)
