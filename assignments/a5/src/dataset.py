...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        document=self.data[idx]#0
        # print("-----------------------------")
        # print(document)
        length=random.randint(4,(self.block_size*7)//8)
        truncatedDocument=document[:length]#1

        truncatedLen=len(truncatedDocument)
        # print(truncatedDocument)
        # print(truncatedLen)
        startIdx=random.randint(1,truncatedLen-2)
        endIdx=random.randint(startIdx+1,truncatedLen-1)
        
        prefix=truncatedDocument[:startIdx]
        masked_content=truncatedDocument[startIdx:endIdx]
        suffix=truncatedDocument[endIdx:]#2

        masked_string=prefix+self.MASK_CHAR+suffix+self.MASK_CHAR+masked_content
        masked_string=masked_string+self.PAD_CHAR*(self.block_size-len(masked_string))#3

        ipt=masked_string[:-1]
        out=masked_string[1:]#4

        x=torch.tensor([self.stoi[c] for c in ipt],dtype=torch.long)
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        #for Debug
        corruption_dataset.__getitem__(4)
    else:
