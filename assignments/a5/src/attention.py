...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        B, T, C = x.size()
        # print(x.shape)
        # calculate XAi values for all heads in batch and move head forward to be the batch dim
        XAi = self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        
        XAi=F.relu(XAi) #(B,nh,T,hs)
