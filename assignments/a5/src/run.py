...
if args.variant == 'vanilla':
    ...
    mconf.setSynthesize(True)
    # print(mconf.synthesizer)
    model=model.GPT(mconf).to(device)
...
if args.function == 'pretrain':
    ...
    #     num_workers=4
    pretrainCorpus=open(args.pretrain_corpus_path).read().strip()
    pretrainDataset=dataset.CharCorruptionDataset(pretrainCorpus,block_size)
    ...
    trainer=trainer.Trainer(model,pretrainDataset,None,tconf)
    trainer.train()
