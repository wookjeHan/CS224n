...
def naiveSoftmaxLossAndGradient(
    ...
    scores=np.exp(np.sum(scores,axis=1))
    scoresMatrix=scores
    
    total=np.sum(scores)
    lossForBack=scores[outsideWordIdx]/total
    loss=-np.log(lossForBack)

    ##backPropForRoute1
    lossForBack=-1/lossForBack #scalar
    loss1=lossForBack/total #scalar
    
    lossExp=np.zeros(N)
    lossExp[outsideWordIdx]=loss1
    ##backPropForRoute2
    loss2=lossForBack*scores[outsideWordIdx]#scalar
    loss2=-loss2/(total**2)
    
    lossExp+=loss2#N,
    lossExp*=scoresMatrix#N,
    lossGrad=np.ones_like(outsideVectors)*lossExp.reshape(N,1)
    gradCenterVec=np.sum(lossGrad*outsideVectors,axis=0)
    gradOutsideVecs=lossGrad*centerWordVec
...
def negSamplingLossAndGradient(
    ...
    ##computing Loss
    positiveNeg=np.sum(outsideVectors[outsideWordIdx]*centerWordVec)
    positiveNeg=sigmoid(positiveNeg)
    positiveCache=positiveNeg
    positiveNeg=np.log(positiveNeg)
    loss=-positiveNeg
    negativeCache=[]
    for i in negSampleWordIndices:
        negativeNeg=-1*np.sum(outsideVectors[i]*centerWordVec)
        negativeNeg=sigmoid(negativeNeg)
        negativeCache.append(negativeNeg)
        negativeNeg=np.log(negativeNeg)
        loss-=negativeNeg
    
    ##computing backForRight
    dLoss1=-1/positiveCache
    dLoss1=dLoss1*positiveCache*(1-positiveCache)
    gradCenterVec=dLoss1*outsideVectors[outsideWordIdx]
    gradOutsideVecs[outsideWordIdx]=dLoss1*centerWordVec

    ##computing backForWrong
    for i in range(len(negSampleWordIndices)):
        dLoss1=-1/negativeCache[i]
        dLoss1=dLoss1*negativeCache[i]*(1-negativeCache[i])
        dLoss1*=-1
        gradCenterVec+=dLoss1*outsideVectors[negSampleWordIndices[i]]
        gradOutsideVecs[negSampleWordIndices[i]]+=dLoss1*centerWordVec
    ### END YOUR CODE
    return loss, gradCenterVec, gradOutsideVecs
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for s in outsideWords:
        outsideWordInd=word2Ind[s]
        lossEnt,gradCenterVecsEnt,gradOutsideVectorsEnt=word2vecLossAndGradient(centerWordVec,outsideWordInd,outsideVectors,dataset)
        loss+=lossEnt
        gradCenterVecs[word2Ind[currentCenterWord]]+=gradCenterVecsEnt
        gradOutsideVectors+=gradOutsideVectorsEnt
    ### END YOUR CODE
