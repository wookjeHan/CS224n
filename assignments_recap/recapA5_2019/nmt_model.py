...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars=self.vocab.src.to_input_tensor_char(source,device=self.device)#(src_len,b,mWord)
        target_padded_chars=self.vocab.tgt.to_input_tensor_char(target,device=self.device)#(src_len,b,mWord)
        ...
        if self.charDecoder is not None:
            ...
            # print(target_padded_chars[1:].shape)
            # print(max_word_len)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X=nn.utils.rnn.pack_padded_sequence(X,source_lengths)#(src_len,b,e)
        enc_hiddens,(h_n,c_n)=self.encoder(X)
        enc_hiddens,_=nn.utils.rnn.pad_packed_sequence(enc_hiddens,batch_first=True,padding_value=self.vocab.src["<pad>"])#(b,src_len,2*h)
        
        h_n=torch.cat((h_n[0,:,:],h_n[1,:,:]),dim=1)#(b,2h)
        c_n=torch.cat((c_n[0,:,:],c_n[1,:,:]),dim=1)#(b,2h)
        init_decoder_hidden=self.h_projection(h_n)#(b,h)
        init_decoder_cell=self.c_projection(c_n)#(b,h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        splitedY=torch.split(Y,1,0)#(1,b,e)*tgt_len
        for Y_t in splitedY:
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        unsqueezedDec=torch.unsqueeze(dec_hidden,2)#(b,h,1)
        e_t=torch.bmm(enc_hiddens_proj,unsqueezedDec)#(b,src_len,1)
        e_t=torch.squeeze(e_t,2)#(b,src_len)
        ...
        unsqueezedAlpha=torch.unsqueeze(alpha_t,1)#(b,1,src_len)
        a_t=torch.bmm(unsqueezedAlpha,enc_hiddens)#(b,1,2h)
    ...
    def save(self, path: str):
        ...
        params = {
            'args': dict(embed_size=self.model_embeddings_source.eWord, hidden_size=self.hidden_size, dropout_rate=self.dropout_rate),
            'vocab': self.vocab,
