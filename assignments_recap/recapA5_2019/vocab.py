...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        word_ids=[[[self.start_of_word]+[self.char2id[ch] for ch in word]+[self.end_of_word] for word in sentence] for sentence in sents]
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padSents=pad_sents_char(word_ids,self.word2id['<pad>'])
        
        sents_var=torch.tensor(padSents,device=device)
