...
def naiveSoftmaxLossAndGradient(
    ...
    ### END YOUR CODE
    scores=np.exp(outsideVectors.dot(centerWordVec.reshape(-1,1)))#(v,1)
    scoresSum=np.sum(scores,axis=0)
    loss=-np.log(scores[outsideWordIdx]/scoresSum)
    
    gradCenterVec=-1*outsideVectors[outsideWordIdx]#(v,1)
    gradCenterVecRes=outsideVectors*scores/scoresSum#(v,d)
    gradCenterVec+=np.sum(gradCenterVecRes,axis=0)

    gradOutsideVecs=np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx]-=centerWordVec
    gradOutsideVecs+=(scores*centerWordVec.reshape(1,-1)/scoresSum)
...
def negSamplingLossAndGradient(
    ...
    answerVec=outsideVectors[outsideWordIdx]
    wrongVecs=outsideVectors[negSampleWordIndices]#(k,d)
    
    loss=-np.log(sigmoid(centerWordVec.dot(answerVec)))
    wrongDots=-np.matmul(wrongVecs,(centerWordVec.reshape(-1,1)))#(k,1)
    loss-=np.sum(np.log(sigmoid(wrongDots)),axis=0)
    
    gradCenterVec=-(1-sigmoid(centerWordVec.dot(answerVec)))*answerVec#(d,)
    gradCenterVecRes=1-sigmoid(wrongDots)#(k,1)
    gradCenterVecRes=wrongVecs*gradCenterVecRes#(k,d)
    gradCenterVec+=(gradCenterVecRes.sum(axis=0))

    gradOutsideVecs=np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx,:]=-(1-sigmoid(centerWordVec.dot(answerVec)))*centerWordVec
    gradOutsideVecsRes=np.matmul((1-sigmoid(wrongDots)),centerWordVec.reshape(1,-1))#(k,d)
    np.add.at(gradOutsideVecs,negSampleWordIndices,gradOutsideVecsRes)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentCenterWordVec=centerWordVectors[centerWordIdx]
    for word in outsideWords:
        outsideWordIdx=word2Ind[word]
        lossTemp,gradCenterVecsTemp,gradOutsideVectorsTemp=word2vecLossAndGradient(currentCenterWordVec,outsideWordIdx,
                                                                                    outsideVectors,dataset)
        loss+=lossTemp
        gradCenterVecs[centerWordIdx]+=gradCenterVecsTemp
        gradOutsideVectors+=gradOutsideVectorsTemp
    ### END YOUR CODE
