...
if args.function == 'pretrain':
    ...
    trainerConfig=trainer.TrainerConfig(max_epochs=650,batch_size=128,learning_rate=6e-3,lr_decay=True,warmup_tokens=512*20,
    ...
    finetuneText = open(args.finetune_corpus_path).read()
    finetune_dataset = dataset.NameDataset(pretrain_dataset, finetuneText)
    trainerConfig=trainer.TrainerConfig(max_epochs=75,batch_size=256,learning_rate=6e-4,lr_decay=True,warmup_tokens=512*20,
                                        final_tokens=200*len(pretrain_dataset)*block_size,num_workers=4,ckpt_path=args.writing_params_path)
    trainer=trainer.Trainer(model,finetune_dataset,None,trainerConfig)
    trainer.train()
    trainer.save_checkpoint()
