...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncatedDoc=document[:length]#1

        truncatedLen=len(truncatedDoc)
        startIdx=random.randint(1,truncatedLen-2)
        endIdx=random.randint(startIdx+1,truncatedLen-1)
        
        prefix=truncatedDoc[:startIdx]
        masked_content=truncatedDoc[startIdx:endIdx]
        suffix=truncatedDoc[endIdx:]#2
        
        masked_string=prefix+self.MASK_CHAR+suffix+self.MASK_CHAR+masked_content
        masked_string+=self.PAD_CHAR*(self.block_size-len(masked_string))
        x=[self.stoi[i] for i in masked_string[:-1]]
        y=[self.stoi[i] for i in masked_string[1:]]
